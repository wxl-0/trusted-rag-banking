...
| 状态 | 当前范围 |
|---|---|
| 已实现并可运行 | 文档解析、混合检索、证据回答、FastAPI REST/SSE、React 问答、Keycloak 登录与访问控制、PostgreSQL 对话历史与滚动摘要、维护者知识库管理，以及在线上传、MinIO 版本工件、Redis 单文档 Worker、Qdrant/BM25 索引、当前版本过滤和并发安全发布 |
...
- **维护者知识库**：知识库维护者可以查看成功/进行中/失败摘要，按文件名搜索、按状态筛选，以每页 10 篇的完整页码翻页，并查看当前版本、上传人与最新任务结果；也可上传单个 DOC、DOCX、PDF、XLS 或 XLSX 文件，由后台完成单文档解析、版本 Chunk 保存、Qdrant/BM25 索引和成功发布。普通成员调用管理接口会被后端拒绝。当前上传入口仍创建新的知识文档，同文档替换上传和版本浏览不在本阶段范围内。
