...
生成层使用 OpenAI 兼容接口，通过 `OPENAI_BASE_URL` 和 `LLM_MODEL` 配置供应商与模型，默认目标为 `deepseek-v4-flash`。问题类型与表格坐标由 TypeSafe System One（`typesafe-sdk`）做类型化判断，使用独立的 `TYPESAFE_API_KEY`，候选选项由代码从题面和索引真实标签枚举。服务端保留完整对话原文，模型调用则按 64K/128K/200K 分层预算选取证据、滚动摘要和最新消息，并预留 8K–16K 输出空间。
...
- **多目标问题分解**：先由类型化判断确定问题走制度、表格、混合还是库外路线，再识别跨文件、多事实、选项比较和季度计算问题；表格问题的来源、行标签、列口径和期间在索引真实标签中选择，无法确定的操作数保持缺失以触发拒答；证据缺失时最多补搜一次。
...
- **受控长对话**：旧消息超过近期历史预算时在 PostgreSQL 更新滚动摘要；受控历史连同回答证据一起进入类型化判断，表格短追问的来源、期间、行列口径由坐标判断在索引真实标签中选出，制度类追问仍走上下文化改写；原始消息完整保留。
...
- 299 道非歧义正式评测题答对 292 题，准确率 97.66%；
- 制度事实类准确率 97.00%，表格题总体准确率 98.99%；
- 证据引用命中率 97.66%。

比赛提交快照（2026-08-21 运行）：

- 299 道非歧义正式评测题答对 291 题，准确率 97.32%；
- 制度事实类准确率 96.50%，表格题总体准确率 98.99%；
- 证据引用命中率 99.00%；
- 100 道开放式专项评测答对 90 题，关键实体错误率 4.92%，库外处理正确率 93.33%。该专项集在类型化判断改动后尚未重跑。
...
TYPESAFE_API_KEY=填写TypeSafe System One密钥
