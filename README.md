...
| 状态 | 当前范围 |
|---|---|
| 已实现并可运行 | 文档解析、结构化切块、Qdrant + BM25 混合检索、RRF 与精排、问题分解、证据覆盖、答案生成/拒答、FastAPI REST/SSE、React 单页问答、Keycloak 登录与访问控制，以及 PostgreSQL 单对话/消息持久化和刷新恢复 |
| 静态原型已完成 | 普通成员/知识库维护者角色视图、左侧历史对话、知识库管理、上传与异步入库状态；登录、账户区和单个当前对话已接入正式 React，其余原型位于 [`prototype/`](prototype/)，尚未接入正式后端 |
| 已确认但尚未开发 | 对话历史列表与管理、服务端 token 预算和摘要、知识文档/版本/任务业务表、Redis 队列、MinIO 原始文件版本，以及网页上传后的解析/索引 |
...
- **单个对话持久化**：正式 React 在首次提问前创建个人对话，SSE 请求只提交对话 ID、幂等请求 ID 和当前问题；服务端校验归属并保存问题、完成回答、证据和时间，刷新或重新登录后可以恢复当前对话。对话列表、标题、搜索、重命名和删除仍待后续实现。
