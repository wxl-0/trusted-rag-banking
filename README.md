...
| 状态 | 当前范围 |
|---|---|
| 已实现并可运行 | 文档解析、混合检索、证据回答、FastAPI REST/SSE、React 问答、Keycloak 登录与访问控制、PostgreSQL 个人对话/消息持久化与历史管理，以及滚动摘要和分层 token 预算 |
| 静态原型已完成 | 普通成员/知识库维护者角色视图、知识库管理、上传与异步入库状态；登录、账户区、问答和个人历史侧栏已接入正式 React，其余原型位于 [`prototype/`](prototype/)，尚未接入正式后端 |
| 已确认但尚未开发 | 知识文档/版本/任务业务表、Redis 队列、MinIO 原始文件版本，以及网页上传后的解析/索引 |
