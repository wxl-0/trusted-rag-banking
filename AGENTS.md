...
- 正式代码已包含 Manifest/命令行建库、类型化问题路由与表格坐标判断、FastAPI/React 问答、Keycloak OIDC 登录与访问控制、PostgreSQL 个人对话/消息持久化、滚动摘要与分层上下文预算。
- `prototype/` 是正式 React 后续页面的视觉与交互基准。登录、账户区、问答、个人历史侧栏、维护者知识库页面、上传弹窗和入库状态已接入正式前端。
- 对话历史管理、知识库管理、维护者批量上传（前端并发调用单文件接口）、活动文档同名拒绝、受鉴权的原件流式下载、MinIO 私有原件与版本 Chunk、Redis 入库 Worker、Qdrant/BM25 版本化索引、受协调锁保护的原子版本发布、幂等重投递、过期任务恢复、暂存清理、五项就绪检查、上传接受审计和安全逻辑撤回已实现；维护者手工重试和永久物理清理尚未实现。
...
- PostgreSQL 保存业务事实，MinIO 保存原始文件及版本，Redis 负责异步任务协调，Qdrant 保存向量索引，BM25 保存关键词索引，TypeSafe System One 只做问题类型与表格坐标的类型化判断；不要让其中一个组件越权承担其他组件职责。
- 问题路由和表格坐标由 `src/judgment.py` 统一判断，候选选项必须由代码从题面或索引真实标签枚举，模型只负责在候选中选择；不得回退到中文语义正则、关键词表或手写指代消解规则。
...
- 不提交 `.env`、真实密码、令牌、客户端密钥、模型缓存、原始资料、Chunk、BM25 文件或 Qdrant 数据。比赛 Manifest、评测题集、逐题报告、Bad Case 回归明细和来源哈希同样不得进入 Git；可以在文档中保留无法还原单题的汇总指标和结论，经确认的 `docs/assets/` 项目展示截图允许提交。`keycloak/realm-export.json` 中明确标注的本地公开演示凭证是唯一例外，不得复用到真实环境。
...
- 从 `src/frontend` 直接执行 `npm run dev` 时，Vite 默认不会读取仓库根目录的
...
- Docker/Compose 前端通过构建参数注入 `VITE_KEYCLOAK_*`，与上述本地开发启动
  路径不同；本地 Vite 漏传环境变量不能作为生产配置失败的证据。
