...
- 正式代码已包含 Manifest/命令行建库、FastAPI/React 问答、Keycloak OIDC 登录与访问控制，以及 PostgreSQL 个人对话/消息持久化和历史管理。
- `prototype/` 是正式 React 后续页面的视觉与交互基准。登录、账户区、问答和个人历史侧栏已接入正式前端；知识库管理、上传与入库状态仍主要是静态原型。
- 对话列表、标题搜索、重命名和逻辑删除已实现；知识文档/版本/任务/审计表、Redis、MinIO、在线异步入库和服务端上下文预算尚未实现。
