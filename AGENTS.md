...
- 正式代码已包含 Manifest/命令行建库、FastAPI/React 问答、Keycloak OIDC 登录与问答访问控制，以及 PostgreSQL 单对话/消息持久化和当前对话恢复。
- `prototype/` 是正式 React 后续页面的视觉与交互基准。登录、账户区和单个当前对话已接入正式前端；历史侧栏、知识库管理、上传与入库状态仍主要是静态原型。
- `conversations` 与 `conversation_messages` 已实现；对话列表/标题/搜索/重命名/删除、知识文档/版本/任务/审计表、Redis、MinIO、在线异步入库和服务端上下文预算尚未实现。
