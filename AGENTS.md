...
- `NFRA-130`（《2023年商业银行主要指标分机构类情况表（季度）》）存在两级行结构解析缺陷：A 列季度分组只在首行有值，B 列才是具体指标；当前 `ExcelParser` 会跳过 A 列为空的后续指标行，导致次级类、可疑类、损失类贷款余额的数值 chunk 缺失，并把首行不良贷款余额错误标成季度。该问题影响 Q036–Q041、Q071–Q073，Q018、Q070 虽可能通过宽松检索答对，但元数据仍不正确且会产生不必要补搜。
- `NFRA-128`（《2023年银行业总资产、总负债（季度）》）也存在缺失数值 chunk：Q074 所需的原始单元格 E8 有值，但当前仅 B8 已入 chunk。具体解析原因尚未定位，后续与其他切块问题一起修复并重新入库；在修复前不使用 Q074 判断检索或推理能力。
- `NFRA-467`（附件8《应当编报保险集团偿付能力报告的公司名单》，对应 Q202）现有 PDF chunk 存在明显截断、丢失上下文的文本，如“定义的保险集团暂不编报偿付能／力报告”，不足以可靠支持正确选项。重切块前先核验原 PDF，确定应修普通文本解析还是增加 OCR；确认修复方式后与其他受影响文件同批重新入库。作为选项引用来源的 `NFRA-490`（规则第19号）现有正文 chunk 不计入待重切文件。
- 暂不立即重切块。先继续排查其他问题并汇总所有会改变 chunk 的修复项，确认范围后统一执行一次切块和向量入库，避免重复重建。
- 已确认执行边界：后续只对确认受影响的 `doc_id` 做单文件解析、旧 chunk 替换和单文件 Qdrant 向量更新，不全量重切、不重新向量化或改动其他文件。所有受影响文件处理完成后统一全量重建一次 BM25；BM25 重建只更新关键词索引，不改变其他文件的 chunk 或向量。
- 实施前需先补充按 `doc_id` 的安全更新能力。不得直接使用当前 `scripts/ingest.py` 做单文件修复，因为它启动时会清空全量 chunk 文件；当前 `scripts/build_index.py` 也只按 collection 总数续传，不能安全替换指定文档。
